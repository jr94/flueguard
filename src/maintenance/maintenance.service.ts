...
export class MaintenanceService {
  ...
  async getOrCreate(deviceId: number): Promise<any> {
    ...
    return {
      ...
      requires_preventive_maintenance: usageHours >= MAINTENANCE_PREVENTIVE_HOURS,
  ...
  async checkAndNotifyMaintenance(deviceId: number) {
    ...
    if (usageHours >= MAINTENANCE_URGENT_HOURS) {
      if (this.shouldNotify(maintenance.last_urgent_notified_at)) {
        this.logger.log(`[Maintenance] Urgent alert sent device=${deviceId} usageHours=${usageHours}`);
        ...
        this.logger.log(`[Maintenance] Urgent alert skipped device=${deviceId} reason=recently_notified`);
        return;
      }
    }
    ...
    if (usageHours >= MAINTENANCE_PREVENTIVE_HOURS) {
      if (this.shouldNotify(maintenance.last_preventive_notified_at)) {
        this.logger.log(`[Maintenance] Preventive alert sent device=${deviceId} usageHours=${usageHours}`);
        ...
        this.logger.log(`[Maintenance] Preventive alert skipped device=${deviceId} reason=recently_notified`);
      }
    }
  }

  private shouldNotify(lastNotifiedAt: Date | null, repeatDays = MAINTENANCE_REPEAT_DAYS): boolean {
  ...
  async handleMaintenanceCron() {
    ...
    this.logger.log(`[Maintenance] Found ${records.length} devices requiring maintenance check`);

    for (const record of records) {
      try {
        await this.checkAndNotifyMaintenance(record.device_id);
      } catch (error) {
        this.logger.error(`[Maintenance] Error checking maintenance for device ${record.device_id}: ${error.message}`);
      }
    }
  }
